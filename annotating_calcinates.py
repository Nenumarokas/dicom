...
def join_images(image: np.ndarray, true_image: np.ndarray, annotated: np.ndarray, selected_layer: int, mark: bool, x: int, y: int, size: int, floodfill: bool):
    ...
    true_slice = cv.addWeighted(true_slice, 0.5, annotated_shown, 0.5, 0)
...
if __name__ == '__main__':
    ...
    calcinate_annotation_file = f'annotations\\{dcm_folder}_calcinates.npy'
    image = read_dicom(f'{os.getcwd()}\\{dcm_folder}')

    if not os.path.exists(annotation_file):
        print('perform vessel annotations first!')
        exit()
    vessel_mask = read_annotations(annotation_file).astype(bool)
    image = np.where(vessel_mask, image, 0)
    ...
    if os.path.exists(calcinate_annotation_file):
        annotated = read_annotations(calcinate_annotation_file)
    ...
    print(np.sum(annotated.astype(int)))
    if np.sum(annotated.astype(int)) < 50:
        exit()
    annotated = np.where(vessel_mask, annotated, False)
    save_annotations(calcinate_annotation_file, annotated)
