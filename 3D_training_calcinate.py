...
class DataLoaderDataset(Dataset):
    ...
    def __getitem__(self, idx):
        ...
        scan = np.load(f'{disk}/vessels/{filename}')
        mask = np.load(f'{disk}/calcinates/{filename}')
...
def prepare_data(folders: list[str], train_size: float, test_size: float, batch_size: int, workers: int):
    filenames = os.listdir(f'{folders[0]}/vessels')
...
if __name__ == '__main__':
    ...
    location_folders = [
        f'/mnt/c/Users/mariu/Downloads/train_dataset/calcinate_dataset/{selected_data_folder}',
        f'/mnt/d/dicom/calcinate_dataset/{selected_data_folder}']
    num_epochs = 200
    batch_size = 64
    workers = 4
    ...
    selected_folder = f'{model_output_folder}/train{last_model_number+1}_calcinate'
